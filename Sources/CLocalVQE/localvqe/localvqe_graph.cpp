...
void ensure_backends_loaded() {
    // Pipit builds ggml with GGML_USE_CPU, so the registry in
    // ggml-backend-reg.cpp registers the CPU backend when it is first used.
    // Upstream this function also
    // calls ggml_backend_load_all() and scans the directory of the shared
    // library (found through dladdr) for backend plug-ins. Pipit links no
    // plug-ins and ships no shared library, so that scan is left out. See
    // UPDATING.md.
}
