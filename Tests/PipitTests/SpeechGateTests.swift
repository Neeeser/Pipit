...
private let windowSeconds = SpeechEvidenceBuilder.levelWindowSeconds
private let speechWindowSeconds = 0.256

/// Builds evidence for a recording of `seconds`, with the given spans
/// written over a background of near-silence on both tracks.
private func evidence(seconds: Double, spans: [Span], detector: String? = "silero") -> SpeechEvidence {
...
private func chunk(_ segments: [(Double, Double, String)]) -> RawTranscriptChunk {
    RawTranscriptChunk(
...
struct SpeechGatePolicyTests {
    @Test("words over audio holding no voice are dropped whatever the levels say")
    func wordsOverAudioHoldingNoVoiceAreDroppedWhateverTheLevelsSay() async throws {
    ...
    @Test("a sentence the detector is sure of is the user's, whatever the far end read")
    func aSentenceTheDetectorIsSureOfIsTheUserSWhateverTheFarEndRead() async throws {
    ...
    @Test("a segment with no letters or digits in it is not words")
    func aSegmentWithNoLettersOrDigitsInItIsNotWords() async throws {
        ...
        #expect(!LocalSpeechPolicy.holdsWords(" ♪"))
        #expect(
            abs((DegenerateTranscriptPolicy.repeatedShare(of: " ♪ ♪ ♪ ♪")) - 0) <= 0.001,
            "expected \(0) ± \(0.001), got \((DegenerateTranscriptPolicy.repeatedShare(of: " ♪ ♪ ♪ ♪")))"
        )
        #expect(
            LocalSpeechPolicy.decide(
                text: " ♪",
    ...
    @Test("one track keeps what the detector heard")
    func oneTrackKeepsWhatTheDetectorHeard() async throws {
        #expect(
            LocalSpeechPolicy.decide(
    ...
    @Test("a machine with no detector keeps every segment")
    func aMachineWithNoDetectorKeepsEverySegment() async throws {
...
struct SpeechGateEvidenceTests {
    ...
    func aReadingCoversTheWholeSpanNotTheWindowItsStartLandsIn() async throws {
        ...
        #expect(
            ...
            "expected \(-12) ± \(0.001), got \(reading.loudestLocalDB) — the loud window is found"
        )
        #expect(
            abs((reading.loudestFarDB ?? 0) - -50) <= 0.001,
            "expected \(-50) ± \(0.001), got \((reading.loudestFarDB ?? 0))"
        )
        #expect(
            abs((reading.speechProbability ?? 0) - 0.99) <= 0.001,
            "expected \(0.99) ± \(0.001), got \((reading.speechProbability ?? 0))"
        )
    }

    @Test("a span past the end of the recording is unmeasured, not silent")
    func aSpanPastTheEndOfTheRecordingIsUnmeasuredNotSilent() async throws {
        ...
        #expect(evidence.reading(from: 30, to: 31) == nil)
    }

    @Test("evidence written with the echo series still reads")
    func evidenceWrittenWithTheEchoSeriesStillReads() async throws {
        ...
        #expect(decoded.micSpeech == [90, 91])
    }

    @Test("a meeting with one track reports no far end")
    func aMeetingWithOneTrackReportsNoFarEnd() async throws {
        ...
        let reading = try #require(evidence.reading(from: 1, to: 2))
        #expect(reading.loudestFarDB == nil)
    }

    @Test("a far end recorded as silence is read as no far end at all")
    func aFarEndRecordedAsSilenceIsReadAsNoFarEndAtAll() async throws {
        ...
        #expect(!evidence.farEndCarriesSignal)
        let reading = try #require(evidence.reading(from: 1, to: 2))
        #expect(reading.loudestFarDB == nil)
    }

    @Test("digital silence reads as the floor rather than as minus infinity")
    func digitalSilenceReadsAsTheFloorRatherThanAsMinusInfinity() async throws {
        #expect(SpeechEvidence.decibels(rms: 0) == Int8(EmptyTranscriptPolicy.silenceFloorDBFS))
        #expect(SpeechEvidence.decibels(rms: 1) == 0)
        #expect(SpeechEvidence.decibels(rms: 0.1) == -20)
    }
}

@Suite("SpeechGate/assembly")
struct SpeechGateAssemblyTests {
    @Test("the standup turn survives and the invented filler after it does not")
    func theStandupTurnSurvivesAndTheInventedFillerAfterItDoesNot() async throws {
    ...
    func aMeetingWithNoEvidenceAssemblesExactlyAsItDidBefore() async throws {
        ...
        #expect(transcript.utterances.count == 1, "kept, for want of anything to judge it by")
    }

    @Test("the far end's own track is never gated")
    func theFarEndSOwnTrackIsNeverGated() async throws {
...
struct SpeechGateMeasurementTests {
    @Test("each track's measurements are moved onto the meeting timeline")
    func eachTrackSMeasurementsAreMovedOntoTheMeetingTimeline() async throws {
        ...
        #expect(
            Array(evidence.remoteLevels.prefix(leadInWindows)) == Array(repeating: floor, count: leadInWindows),
            "the later track reads as nothing until it started"
        )
        let afterPadding = Array(evidence.remoteLevels.dropFirst(leadInWindows))
        #expect(!afterPadding.isEmpty, "and its own audio follows the padding")
        ...
        #expect(evidence.micSpeech.isEmpty, "no detector, no readings")
        #expect(evidence.detector == nil)
    }

    @Test("a recording with no far end carries no far-end series")
    func aRecordingWithNoFarEndCarriesNoFarEndSeries() async throws {
        ...
        #expect(evidence.remoteLevels.isEmpty, "no far end was recorded")
    }

    @Test("a far end that stopped recording first reads as absent after it stopped")
    func aFarEndThatStoppedRecordingFirstReadsAsAbsentAfterItStopped() async throws {
        ...
        let during = try #require(evidence.reading(from: 2, to: 3))
        #expect(during.loudestFarDB != nil, "measured while both tracks ran")
        let after = try #require(evidence.reading(from: 40, to: 41))
        #expect(after.loudestFarDB == nil, "nothing recorded to read")
        #expect(
            LocalSpeechPolicy.decide(text: "so that is the plan", reading: after) == .spoken,
            "the detector decides"
        )
    }

    @Test("evidence already on disk is never measured over")
    func evidenceAlreadyOnDiskIsNeverMeasuredOver() async throws {
    ...
    @Test("a meeting is measured once, and only where the gate can use it")
    func aMeetingIsMeasuredOnceAndOnlyWhereTheGateCanUseIt() async throws {
        ...
        await pipeline.process(meetingID: meeting.metadata.id)
        let written = try #require(meeting.store.readSpeechEvidence())
        #expect(!written.micLevels.isEmpty, "a call is measured")
